...
export default async function LoadedUpdateInputs({ id }: { id: string }) {
    const [currentTeamData, possibleMembers] = await Promise.all([getCurrentTeamData(id), getWrestlersWithoutTeam()]);
        ...
        <>
            <Boxed title={'Datos del equipo'} w={'100'}>
                <div className="flex center astart gap-small">
                    ...
                    <Input required={true} max={150} label={'Nombre'} name={'name'} value={currentTeamData.name} />
                    <NumberInput
                        type={'number'}
                        max={3}
                        required={true}
