...
export default function useDraft(wrestlers: Wrestler[]) {
    ...
    const handleFormSubmition = (datas: any) => {
        ...
        const selectedWrestler = wrestlers.find(
            (wrestler) => wrestler.id === selectedID
        );
        const foundInSelection = state.userSelection.find(
            (wrestler) => wrestler.id === selectedID
        );

        if (!selectedWrestler || foundInSelection) return;
        const listAfterSelection = state.draftWrestlers.filter(
            (wrestler) => wrestler.id !== selectedID
        );
        const rawBrandSelection = chooseRawWrestler(
            selectedWrestler,
            listAfterSelection
        );
        ...
        setState((prev: DraftStore) => {
            return {
                ...prev,
                draftWrestlers: prev.draftWrestlers.filter(
                    (wrestler: Wrestler) => wrestler.id !== selectedID
                ),
                // userSelection: [...prev.userSelection, selectedWrestler],
                raw: rawBrandSelection
                    ? [...prev.raw, rawBrandSelection]
                    : [...prev.raw],
    ...
    const chooseRawWrestler = (
        smackdownPick: Wrestler,
        listAfterSmackdownPick: Wrestler[]
    ) => {
        if (listAfterSmackdownPick.length <= 0) {
            return false;
        }
        const genderList = listAfterSmackdownPick.filter(
            (it) => it.sex === smackdownPick.sex
        );
        const finalGender =
            genderList.length <= 0 ? listAfterSmackdownPick : genderList;
        const overallList = finalGender.filter(
            (it) =>
                it.overall >= smackdownPick.overall - 5 &&
                it.overall <= smackdownPick.overall + 5
        );
    ...
    return {
        ...
        raw: state.raw,
        removeSelectText: state.removeSelectText,
        handleFormSubmition,
    };
}
